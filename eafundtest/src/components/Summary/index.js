...
function Summary({ label, rows }) {
  ...
  let totalPayout = rows
    ? rows.reduce((acc, item) => {
        return acc + currencyToNumber(item["Payout amount"]);
      }, 0)
    : 0;
  let median = rows ? computeMedianRating(rows) : 0;
  let positiveRatings = rows
    ? rows.filter((item) => item.Rating > 0).length
    : 0;
  ...
  return (
    <div className="infoTile">
      ...
      <div>
        ...
        <p>
          Sum of payout amount of grants that have a positive rating:{" "}
          {payoutPositiveRatings.toFixed(2)}
        </p>
      </div>
    </div>
  );
